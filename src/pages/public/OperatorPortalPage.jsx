    ...
    <div className="bg-white">
      ...
      <section className="px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 pb-0"><div className="relative py-10 sm:py-14 px-5 sm:px-8 lg:px-10 rounded-2xl overflow-hidden bg-gradient-to-br from-[#00458B] to-[#003366]">
        ...
        <div ref={heroRef} className="relative max-w-3xl mx-auto text-center">
          ...
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white">Spectrum Management Portal</h1>
          <p className="text-white/60 mt-3 text-sm sm:text-base max-w-xl mx-auto">Register your company, manage radio frequency licences, and interact with BOCRA's spectrum management system.</p>
        </div>
      </div></section>
          ...
          {user ? (
            <>
              <button onClick={() => setView('dashboard')} className="bg-white border-2 border-gray-200 rounded-2xl p-8 hover:border-[#6BBE4E] hover:shadow-lg transition-all text-left group">
                ...
                <h2 className="text-lg font-bold text-bocra-slate group-hover:text-[#6BBE4E]">Go to Portal</h2>
                <p className="text-sm text-bocra-slate/50 mt-2">Signed in as {user.email}. Access your dashboard, manage licences, and track applications.</p>
                <span className="text-sm text-[#6BBE4E] font-medium flex items-center gap-1 mt-4">Enter Dashboard <ArrowRight size={14} /></span>
              </button>
              <button onClick={() => setView('register')} className="bg-white border-2 border-gray-200 rounded-2xl p-8 hover:border-[#00458B] hover:shadow-lg transition-all text-left group">
                ...
                <h2 className="text-lg font-bold text-bocra-slate group-hover:text-[#00458B]">Register New Company</h2>
                <p className="text-sm text-bocra-slate/50 mt-2">Register a different company to apply for radio communications and spectrum licences.</p>
                <span className="text-sm text-[#00458B] font-medium flex items-center gap-1 mt-4">Register <ArrowRight size={14} /></span>
              </button>
            </>
          ) : (
            <>
...
function CompanySearch() {
  ...
  return (
    <div className="bg-bocra-off-white rounded-2xl p-6">
      <h3 className="text-sm font-bold text-bocra-slate mb-1">Existing Customer Search</h3>
      <p className="text-xs text-bocra-slate/40 mb-4">Search by company name or customer number to check registration status</p>
      <div className="relative">
        <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
        <input type="text" value={q} onChange={e => setQ(e.target.value)} placeholder="Company name or customer number..."
          ...
          {results.map(c => (
            <div key={c.customer} className="bg-white rounded-xl p-4 border border-gray-100">
              <div className="flex items-start justify-between">
                ...
                <span className="px-2 py-1 bg-green-100 text-green-700 text-[10px] font-bold rounded-full">Registered</span>
              </div>
      ...
      {q.length >= 2 && results.length === 0 && (
        <p className="text-xs text-gray-400 mt-3 text-center">No registered company found for "{q}"</p>
      )}
    </div>
  );
}
  ...
  if (success) return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#00458B] to-[#003366] p-6">
      <div className="bg-white rounded-2xl p-8 max-w-md w-full text-center">
        ...
        <h2 className="text-xl font-bold text-bocra-slate">Registration Successful!</h2>
        <p className="text-sm text-bocra-slate/50 mt-2 mb-4">Your operator account has been created.</p>
        <div className="bg-[#00A6CE]/5 border border-[#00A6CE]/20 rounded-xl p-4 text-left mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-bocra-slate">Verify your email</p>
              <p className="text-xs text-bocra-slate/50 mt-1">We've sent a verification link to <strong className="text-bocra-slate">{form.email}</strong>. Please check your inbox (and spam folder) and click the link to activate your account.</p>
              <p className="text-[10px] text-bocra-slate/30 mt-2">You won't be able to sign in until your email is verified.</p>
            </div>
          </div>
        </div>
        <button onClick={() => setView('login')} className="w-full py-3 bg-[#00458B] text-white rounded-xl font-medium text-sm hover:bg-[#003366] transition-all">Go to Sign In</button>
      </div>
    </div>
  );
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div className="bg-white rounded-2xl overflow-hidden">
          <div className="bg-[#00458B] p-6 text-center">
            ...
            <h2 className="text-xl font-bold text-white">Create Operator Account</h2>
            <p className="text-white/50 text-xs mt-1">Register to access BOCRA spectrum management services</p>
          </div>
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-2xl overflow-hidden">
          <div className="bg-[#00458B] p-6 text-center">
            ...
            <p className="text-white/50 text-xs mt-1">Access your operator dashboard</p>
          </div>
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <p className="text-xs text-center text-gray-400">Don't have an account? <button type="button" onClick={() => setView('register')} className="text-[#6BBE4E] font-medium hover:underline">Create Account</button></p>
          </form>
        </div>
      </div>
    </div>
...
function OperatorDashboard({ operator, user, signOut, setView }) {
  ...
  return(<div className="bg-bocra-off-white min-h-screen">
    <div className="bg-[#00458B] text-white"><div className="section-wrapper py-3 flex items-center justify-between"><div className="flex items-center gap-3"><Radio size={18} className="text-[#00A6CE]"/><span className="text-sm font-bold">ASMS-WebCP</span><span className="text-xs text-white/40 hidden sm:inline">Operator Portal</span></div><div className="flex items-center gap-3">{profilePic?<img src={profilePic} alt="" className="w-7 h-7 rounded-full object-cover border-2 border-white/20"/>:<div className="w-7 h-7 rounded-full bg-[#00A6CE] flex items-center justify-center text-white text-xs font-bold">{ini}</div>}<span className="text-xs text-white/60 hidden sm:inline">{name}</span><button onClick={handleLogout} className="text-xs text-white/40 hover:text-white flex items-center gap-1"><LogOut size={12}/> Sign Out</button></div></div></div>
